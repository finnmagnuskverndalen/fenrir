    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
        <input
          ...
          placeholder="search CVE, title, description..."
          style={{
            background: '#0d0d0d', border: '1px solid #2a2a2a', borderRadius: 4,
            color: '#e5e5e5', padding: '5px 10px', fontSize: 12, outline: 'none', width: 220,
          }}
        />
        {SEVERITIES.map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{
            background: filter === s ? (severityColor[s] || '#555') + '22' : 'transparent',
            border: `1px solid ${filter === s ? (severityColor[s] || '#555') : '#2a2a2a'}`,
            borderRadius: 4,
            color: filter === s ? (severityColor[s] || '#aaa') : '#444',
          ...
          return (
            ...
            >
              ...
              <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {f.cve_id && (
                  <span style={{ color: '#4a9eff', fontSize: 11, fontFamily: 'monospace', flexShrink: 0 }}>
                    {f.cve_id}
                  </span>
                )}
                ...
                <span style={{ color: '#2a2a2a', fontSize: 10, flexShrink: 0 }}>
                  {isOpen ? '▲' : '▼'}
                </span>
              </div>
              ...
              {isOpen && (
                <div style={{ borderTop: '1px solid #1a1a1a', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {f.description && (
                    <div>
                      <div style={{ color: '#333', fontSize: 10, marginBottom: 4 }}>description</div>
                      <div style={{ color: '#888', fontSize: 12, lineHeight: 1.6 }}>{f.description}</div>
                    </div>
                  )}
                  ...
                  {f.ai_analysis && (
                    <div style={{
                      ...
                      <div style={{ color: '#4a9eff', fontSize: 10, marginBottom: 6, fontWeight: 500 }}>
                        AI analysis — deepseek
                      </div>
                      <div style={{ color: '#8888cc', fontSize: 12, lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                        {f.ai_analysis}
                      </div>
                    </div>
                  )}
